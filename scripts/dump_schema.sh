...
# Enforce sslmode=require (harmless if already present)
DBURL="$DATABASE_URL"
...
ALLOW_PARTIAL=${ALLOW_PARTIAL:-0}   # set to 1 in the workflow to not fail the job on a few bad tables
...
# 3) Per-table JSON dumps (continue past failures)
