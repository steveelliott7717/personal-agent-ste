...
# ------------------------- OpenAI embeddings -------------------------
def openai_embeddings_create(model: str, inputs: List[str], timeout: int = 90, max_retries: int = 4) -> List[List[float]]:
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            msg = ""
            try: msg = e.read().decode("utf-8")
            except Exception: pass
            if e.code in (429, 500, 502, 503, 504) and attempt < max_retries:
                print(f"[emb] HTTP {e.code}; retrying in {backoff:.1f}s (attempt {attempt}/{max_retries})")
                time.sleep(backoff); backoff *= 1.8
                continue
            raise RuntimeError(f"OpenAI error {e.code}: {msg}") from None
        except URLError as e:
            if attempt < max_retries:
                print(f"[emb] network error; retrying in {backoff:.1f}s (attempt {attempt}/{max_retries})")
                time.sleep(backoff); backoff *= 1.8
                continue
...
DEFAULT_EXTS = {".py",".ts",".tsx",".js",".jsx",".vue",".sql",".json",".md",".yml",".yaml",".toml",".ini",".env",".txt"}
...
def chunk_lines(lines: List[str], max_lines: int = 80, overlap: int = 10) -> Iterable[Tuple[int,int,str]]:
    n = len(lines)
    if n == 0:
        return
    i = 0
...
def main():
    ...
    ap = argparse.ArgumentParser(description="Index repo into Supabase with embeddings (progress logging).")
    ...
    ap.add_argument("--max-chars", type=int, default=3000, help="Max chars per chunk text before piece truncation")
    ...
    ap.add_argument("--incremental", action="store_true", help="Skip unchanged files based on file SHA")
    ...
    for bi, batch_inputs in enumerate(batches, start=1):
        ...
        for k in range(batch_len):
            ...
            rows.append({
                ...
                "embedding": embeddings[k], "dims": EMBED_DIMS,
            })
        ...
        print(f"[embed {bi}/{total_batches}] ok in {dt:.2f}s (rate {rate:.2f} pcs/s), supabase {dtsup:.2f}s")
