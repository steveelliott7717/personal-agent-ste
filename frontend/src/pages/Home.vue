<template>
  <main class="min-h-screen max-w-2xl mx-auto p-6 space-y-4">
    <header class="flex items-center justify-between">
      ...
      <nav class="text-sm">
        <RouterLink class="underline" :to="{ name: 'repo' }"
          >Open GPT-5 RMS Chat →</RouterLink
        >
      </nav>
    </header>
    ...
    <section class="space-y-2">
      ...
      <div class="flex gap-2">
        ...
        <button class="px-3 py-2 rounded border" @click="clearOut">
          Clear
        </button>
      </div>
    </section>
    ...
    <section
      v-if="error"
      class="border border-red-300 bg-red-50 p-3 rounded text-sm"
    >
    ...
    <section
      v-if="result"
      class="border p-3 rounded text-sm whitespace-pre-wrap break-words"
    >
...
async function run() {
  ...
  loading.value = true
  try {
    result.value = await sendRequest(query.value)
  } catch (e) {
