...
export default {
  ...
  methods: {
    async logTask() {
      await axios.post('/api/request', new URLSearchParams({ query: `log grooming ${this.task}` }))
      this.loadLogs()
    },
    async loadLogs() {
      const res = await axios.post('/api/request', new URLSearchParams({ query: 'show grooming' }))
