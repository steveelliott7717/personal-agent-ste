...
export default {
  ...
  methods: {
    async logWorkout() {
      await axios.post('/api/request', new URLSearchParams({ query: `log workout ${this.workout}` }))
      this.loadWorkouts()
    },
    async loadWorkouts() {
      const res = await axios.post('/api/request', new URLSearchParams({ query: 'show workouts' }))
      this.workouts = res.data.result
    }
  },
  mounted() { this.loadWorkouts() }
}
</script>
