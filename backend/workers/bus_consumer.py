...
def _get_offset():
    ...
    if res:
        return res[0]["last_event_time"]
    supabase.table("event_offsets").insert({"consumer": CONSUMER}).execute()
    return "epoch"


def _set_offset(ts_iso):
    supabase.table("event_offsets").upsert(
        {"consumer": CONSUMER, "last_event_time": ts_iso}
    ).execute()
...
def run_once(limit=200):
    topic_map = (
        _get_map()
    )  # {"meal.done":"plugins.nutrition.handlers:handle_meal_done", ...}
    if not topic_map:
        return
