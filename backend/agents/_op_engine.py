    ...
    for step in ops:
        ...
        if op == "insert":
            ...
            if isinstance(values, dict):
                values = [values]
            out.append(supabase.table(table).insert(values or []).execute().data or [])
            continue
        ...
        if op == "upsert":
            ...
            if isinstance(values, dict):
                values = [values]
            out.append(supabase.table(table).upsert(values or []).execute().data or [])
            continue
