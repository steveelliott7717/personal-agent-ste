...
def _commit_and_push(
    branch_name: str, commit_message: str, run_id: str, pr_title: str, pr_body: str
...
def _deletion_ratio(unified: str) -> float:
    """Roughly estimate how destructive a unified diff is."""
    removed = kept = 0
...
def run_update_pipeline(
    ...
    *,
    ...
    if isinstance(task.get("change_spans"), list) and task["change_spans"]:
        ...
        acceptance_tests = task.get("acceptance_tests", [])  # <-- add this line
        ...
        try:
            ...
            acceptance_tests = list(plan_json.get("acceptance_tests") or [])
            _ev(
                "plan.generated",
    ...
    for idx, span in enumerate(change_spans):
        ...
        resolution_details = (
            resolution_method
            if isinstance(resolution_method, dict)
            else {"method": resolution_method}
        )
        if resolution_details.get("method") == "anchor_repair":
    ...
    # ---------------------------
    applied = False
    branch_out: Optional[str] = None
    pr_url: Optional[str] = None

    # Always create a branch and PR if there are changes.
    branch_name = f"agent/{agent_slug}-{time.strftime('%Y%m%d')}-{uuid.uuid4().hex[:6]}"

    # Apply the patch to a new branch
    applied_ok, apply_msg = _apply_patch(patch_text, run_id)
    if not applied_ok:
        _ev("apply.failed", error=apply_msg)
        _finish_run(run_id, "error", error=f"Failed to apply patch: {apply_msg}")
        return {
            "ok": False,
            "patch": patch_text,
            "events": events,
    ...
    lint_ok, lint_result = _run_linter(run_id)
    if not lint_ok:
        _ev("lint.failed", details=lint_result)
        _finish_run(run_id, "error", error="Linting failed")
        return {
            "ok": False,
            "patch": patch_text,
            "events": events,
    ...
    test_ok, test_result = _run_tests(run_id, acceptance_tests)
    if not test_ok:
        _ev("test.failed", details=test_result)
        _finish_run(run_id, "error", error="Tests failed")
        return {
            "ok": False,
            "patch": patch_text,
            "events": events,
    ...
    if not deps_ok:
        _ev("deps.failed", details=deps_result)
        _finish_run(run_id, "error", error="Dependency check failed")
        return {
            "ok": False,
            "patch": patch_text,
            "events": events,
    ...
    # --- Build evidence summary for commit message ---
    changed_files_count = len(diffs)
    del_ratio = _deletion_ratio(patch_text)
    anchors_used = resolution_details.get("anchors_used", [])
    evidence_summary = (
        f"Changed files: {changed_files_count}\n"
        f"Deletion ratio: {del_ratio:.1%}\n"
        f"Anchors used: {', '.join(anchors_used) if anchors_used else 'none'}"
    )
    commit_message = f"feat: apply automated changes for task\n\n{evidence_summary}\n\nRun ID: {run_id}"
    pr_body = f"""This PR was automatically generated by the `{agent_slug}` agent for run_id `{run_id}`.
...
- [ ] The changes address the task requirements.
- [ ] All automated checks (`Lint`, `Tests`, `Deps`) have passed.
- [ ] The patch is minimal and does not introduce unrelated changes.
"""
    pushed_ok, push_result = _commit_and_push(
        branch_name,
        commit_message,
        run_id,
        pr_title=task.get("task", "Untitled"),
        pr_body=pr_body,
    )

    if pushed_ok:
        branch_out = push_result.get("branch")
        pr_url = push_result.get("pr_url")
        _ev("commit.success", branch=branch_out, pr_url=pr_url)
        summary = f"PR created for branch {branch_out}: {pr_url}"
        _finish_run(run_id, "ok", summary=summary)
        print(f"--- Repo update pipeline END (run={run_id}) PR created: {pr_url} ---")
    else:
        _ev("commit.failed", error=push_result.get("error"))
        summary = f"Failed to push changes: {push_result.get('error')}"
        _finish_run(run_id, "error", error=summary)
        return {
            "ok": False,
            "patch": patch_text,
            "events": events,
