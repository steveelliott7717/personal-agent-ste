    ...
    *,
    ...
    if apply_changes:
        args = {
            ...
            "pr_title": f"Automated changes for task: {task.get('task', 'Untitled')}",
        ...
        pushed_ok, push_result = _commit_and_push(
            args["branch_name"], args["commit_message"], run_id
        )
        if not pushed_ok:
        ...
        # Log the final success state
        _finish_run(
            run_id, "ok", summary=f"PR created for branch {branch_out}: {pr_url}"
        )
